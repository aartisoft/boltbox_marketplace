...
class paypal_lib {
	...
	function __construct()
	{
		...
		$records = $this->CI->db->get('paypal_details',0,1)->row_array();
		if(!empty($records)){

			if(strtolower($records['developement']) == 'live'){
				$sanbox_live = 'live';
				$sanbox_live_businessEmail = (!empty($records['email']))?$records['email']:'';
			}
			if(strtolower($records['developement']) == 'sandbox'){
				$sanbox_live = 'sandbox';
				$sanbox_live_businessEmail = (!empty($records['sandbox_email']))?$records['sandbox_email']:'';
			}
		...
		// values can be overwritten by the calling script.
		 $businessEmail = !empty($sanbox_live_businessEmail)?$sanbox_live_businessEmail:$this->CI->config->item('business');
